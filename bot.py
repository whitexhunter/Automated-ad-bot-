...
async def auto_token_command(ctx, email: str = None, password: str = None):
    ...
    if not email or not password:
        embed = discord.Embed(
            ...
            description="**Usage:** `!autotoken email password`\n\n"
                       "⚠️ **WARNING:** Use temporary password only!",
    ...
    processing_msg = await ctx.send("🔄 Getting token... (30-60 seconds)")
    ...
    try:
        ...
        try:
            ...
            await processing_msg.edit(
                content="❌ Selenium not installed on server!\n"
                       "Add to requirements.txt and redeploy."
            )
            return
        
        # Import our token getter
        try:
            from token_getter import get_discord_token
        except ImportError:
            await processing_msg.edit(
                content="❌ token_getter.py not found!"
            )
            return
        
        # Run in background thread
        def get_token_sync():
            return get_discord_token(email, password)
        ...
        if token:
            # Send to DM
            try:
                await ctx.author.send(f"✅ Token: `{token[:50]}...`")
                ...
                if user_key in manager.user_tokens:
                    ...
                    await ctx.author.send("✅ Token saved to your config!")
                
                await processing_msg.edit(content="✅ Token sent to your DMs!")
                
            except discord.Forbidden:
                await processing_msg.edit(content="❌ Enable DMs to receive token!")
        else:
            await processing_msg.edit(
                content="❌ Failed to get token.\n"
                       "Possible reasons:\n"
                       "• Wrong credentials\n"
                       "• 2FA enabled\n"
                       "• Chrome not installed on server\n"
                       "• Use manual method: `!gettokenhelp`"
            )
